...
class NegativeScenariosIntegrationTest : StringSpec({
    ...
    beforeSpec {
        ...
        coEvery { aiService.generateWorkoutPlan(any()) } returns WorkoutPlan(
        ...
        coEvery { aiService.analyzeFeedback(any(), any()) } returns ActualPerformance(
            rawFeedback = "ok",
    ...
    "Negative: нерелевантный фидбек (мусор) не ломает флоу, тренировка завершается (возможно, без объема)" {
        runBlocking {
            ...
            val w = Workout(
                id = wId, 
                userId = userId, 
                status = WorkoutStatus.IN_PROGRESS, // Сначала IN_PROGRESS
                plan = plan, 
            ...
            val feedbackMsg = TelegramUpdate(
                update_id = 30,
    ...
    "Negative: Prompt Injection в фидбеке не ломает систему (эмуляция ответа AI)" {
        runBlocking {
            ...
            val wId = java.util.UUID.randomUUID().toString()
            val w = Workout(
                id = wId, userId = userId, status = WorkoutStatus.IN_PROGRESS, plan = plan, actualPerformance = null, 
                timing = WorkoutTiming(java.time.Instant.now(), null, null), aiLog = AILog(0, "gpt", 0, null, null)
            )
            workoutRepository.save(w)
