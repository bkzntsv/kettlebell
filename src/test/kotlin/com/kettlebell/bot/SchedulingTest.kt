...
class SchedulingTest {
    ...
    @Test
    fun `handleScheduleCommand transitions to SCHEDULING_DATE state`() = runTest {
        val userId = 123L
        val chatId = 456L
    ...
    @Test
    fun `manual date input schedules workout correctly`() = runTest {
        val userId = 123L
        val chatId = 456L
    ...
    @Test
    fun `quick schedule button 'tomorrow' schedules for next day 18-00`() = runTest {
        val userId = 123L
        val chatId = 456L
    ...
    @Test
    fun `checkReminders sends 1h reminder`() = runTest {
        val now = Instant.now()
        ...
        val user = UserProfile(
            ...
            scheduling = com.kettlebell.model.UserScheduling(
    ...
    @Test
    fun `checkReminders generates workout when 5 mins left and user is IDLE`() = runTest {
        val now = Instant.now()
    ...
    @Test
    fun `start_workout button clears scheduling`() = runTest {
        val userId = 123L
        val chatId = 456L
        val workoutId = "w1"
